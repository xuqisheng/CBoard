...
# remote download
RUN wget http://www.trieuvan.com/apache/tomcat/tomcat-8/v8.5.29/bin/apache-tomcat-8.5.29.tar.gz -P install \
    && wget https://bitbucket.org/ariya/phantomjs/downloads/phantomjs-2.1.1-linux-i686.tar.bz2 -P install

RUN tar -zxf install/apache-tomcat-8.5.29.tar.gz -C /opt \
    && tar -jxf install/phantomjs-2.1.1-linux-i686.tar.bz2 -C /opt \
    && ln -s /opt/apache-tomcat-8.5.29 /opt/apache-tomcat \
